...
//
// Either swipe direction just advances to the next lettering, wrapping
// after the 7th back to the 1st (0..6 has no "back to plain text"
// step)—matching what was asked for: repeat the gesture and it moves
// to another one, round and round. The choice is kept in
// sessionStorage, so it survives closing/reopening the mobile menu (and
// any other in-session navigation) but resets on a fresh tab/session,
// same lifetime as the rest of the site's per-viewer state.
(function () {
  ...
  var currentIndex = -1; // -1 = showing the plain text wordmark

  function showLettering(index) {
    currentIndex = index;
    img.src = urls[index];
  ...
  if (stored !== null) {
    ...
    if (restored >= 0 && restored < urls.length) showLettering(restored);
  }
  ...
  brand.addEventListener('touchend', function (e) {
    ...
    e.preventDefault();
    showLettering((currentIndex + 1) % urls.length);
