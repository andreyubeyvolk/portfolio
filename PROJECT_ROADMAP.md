...
### Phase 3.5 — Polish and pre-launch (while still in HTML)

**Images:**
- Add `loading="lazy"` to all `<img>` tags except first visible image per page (easy perf win)
- Convert all photos to WebP (80–85% quality, target <400KB for covers, <80KB for thumbs)
- Batch conversion script planned: `node scripts/convert-images.js ./folder` — outputs WebP, keeps originals

**Browser QA:**
- Test in Safari desktop + mobile Safari on iPhone (most likely to break)
- Test in Firefox, Chrome, Edge
- Note: viewing site via raw GitHub link is not reliable for testing — use Vercel deploy instead
- Known: About page layout needs checking on mobile (reported visual issues)

**Missing basics:**
- Favicon: generate from logo (16×16, 32×32, 180×180 for iOS) — use favicon.io
- 404 page: custom `404.html` with nav links so users don't get stuck
- `robots.txt`: one-liner in root, tells Google what to index

**Reel video player (custom):**
- Video file lives in `assets/` (50–150MB is fine for hosting)
- Custom HTML5 `<video>` overlay: play/pause, progress bar, mute, fullscreen, collapse
- Two quality versions: full for desktop, lighter for mobile
- No external dependencies — pure JS overlay on native `<video>` element
- Design the player controls UI before building

**NDA-protected case studies:**
- Card in grid looks normal but clicking opens password prompt (inline or overlay)
- Password checked in JS — sufficient for portfolio, not bank-level security
- Client gets password directly from you, random visitors see locked state
- Design the card locked state + password UI when ready to build
- Cleaner implementation in Nuxt (middleware), but doable in HTML too

**SEO and analytics (after going live on real domain):**
- Add `<meta name="description">` to every page
- Add Open Graph tags (`og:title`, `og:description`, `og:image` 1200×630px, `og:url`)
- Register on Google Search Console, submit sitemap.xml
- Connect Google Analytics 4 (one script tag per HTML file now, once in layout after Nuxt)
- ⚠️ Reminder: SEO and analytics only make sense after real domain + hosting

**OG preview images:**
- Create 1200×630px cover image for each section (Home, Inhouse, Brands, Archive, About)
- Shows as card when link is shared in Telegram, LinkedIn, iMessage etc.

**Contact form:**
- Decision: skip. Direct Telegram + email links are more authentic for creative portfolio.
- Clients who want to reach out will use the links. Form adds friction, not value here.
...
- Sitemap.xml: auto-generated by Nuxt plugin (`@nuxtjs/sitemap`)
- Multilingual (idea, not priority): `@nuxtjs/i18n` plugin — EN + RU versions, texts in separate files, zero layout impact. Only consider after all content is finalized in EN.
