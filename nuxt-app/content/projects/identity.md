---
slug: identity
section: inhouse
title: The Future of Digital Identity
description: The Future of Digital Identity visual identity by Andrey Ubeyvolk—Sumsub Multiverse turned inward, from structure to selfhood.
cover:
  type: image
  src: /assets/inhouse/identity/identity-01.webp
  width: 2160
  height: 1440
about: "Digital identity now runs fintech, crypto, and regulated industries. SingPass, ID.me—billions already log in through one verified profile. Juniper Research expects 6.5 billion digital ID users by 2026. But the core tension stays unresolved: private, easy, and compliant, all at once. For Sumsub Multiverse: The Future of Digital Identity (London 2025), we made that tension feel real. Not a system problem. A personal one—being verified, constantly."
gallery:
  - type: wide
    item:
      type: image
      src: /assets/inhouse/identity/identity-02.webp
      width: 2160
      height: 1440
      alt: Digital identity visual 02
  - type: pair
    items:
      - type: video
        src: /assets/inhouse/identity/identity-03.mp4
        width: 1080
        height: 1080
        player: bare
      - type: image
        src: /assets/inhouse/identity/identity-04.webp
        width: 1080
        height: 1440
        alt: Digital identity visual 04
  - type: wide
    item:
      type: image
      src: /assets/inhouse/identity/identity-05.webp
      width: 2160
      height: 1440
      alt: Digital identity visual 05
  - type: wide
    item:
      type: image
      src: /assets/inhouse/identity/identity-06.webp
      width: 2160
      height: 1440
      alt: Digital identity visual 06
  - type: video
    item:
      type: video
      src: /assets/inhouse/identity/identity-07.mp4
      width: 1920
      height: 1080
      player: bare
  - type: pair
    items:
      - type: video
        src: /assets/inhouse/identity/identity-08.mp4
        width: 1080
        height: 1080
        player: bare
      - type: image
        src: /assets/inhouse/identity/identity-09.webp
        width: 1080
        height: 1440
        alt: Digital identity visual 09
        note: Consistent AI-generated character style
  - type: wide
    item:
      type: image
      src: /assets/inhouse/identity/identity-10.webp
      width: 2160
      height: 1440
      alt: Digital identity visual 10
  - type: pair
    items:
      - type: image
        src: /assets/inhouse/identity/identity-11.webp
        width: 1080
        height: 1440
        alt: Digital identity visual 11
      - type: image
        src: /assets/inhouse/identity/identity-12.webp
        width: 1080
        height: 1440
        alt: Digital identity visual 12
  - type: wide
    item:
      type: image
      src: /assets/inhouse/identity/identity-13.webp
      width: 2160
      height: 1440
      alt: Digital identity visual 13
  - type: pair
    items:
      - type: image
        src: /assets/inhouse/identity/identity-14.webp
        width: 1080
        height: 1440
        alt: Digital identity visual 14
      - type: image
        src: /assets/inhouse/identity/identity-15.webp
        width: 1080
        height: 1440
        alt: Digital identity visual 15
  - type: wide
    item:
      type: image
      src: /assets/inhouse/identity/identity-16.webp
      width: 2160
      height: 1440
      alt: Digital identity visual 16
  - type: video
    item:
      type: video
      src: /assets/inhouse/identity/identity-conf.mp4
      width: 1920
      height: 1080
      player: full
  - type: pair
    items:
      - type: image
        src: /assets/inhouse/identity/identity-17.webp
        width: 1080
        height: 1440
        alt: Digital identity visual 17
      - type: image
        src: /assets/inhouse/identity/identity-18.webp
        width: 1080
        height: 1440
        alt: Digital identity visual 18
  - type: wide
    item:
      type: image
      src: /assets/inhouse/identity/identity-19.webp
      width: 2160
      height: 1440
      alt: Digital identity visual 19
challenge: "Sumsub Multiverse already had its own design language. Layered visuals. Stepped typography. Built to show the hidden depths of fraud prevention. For the Digital Identity chapter, that language had to shift. Not systems anymore—people. The challenge: keep the Multiverse grammar intact, but turn it inward. From structure to selfhood."
solution: "We kept the core visual logic—layers, structure, depth. Then pointed it at the human figure instead of the system. Fragmented, glitched silhouettes became the centerpiece. Faces shimmer under noise, cyberpunk-inspired. Identity reads as unresolved—always mid-verification. Around it: biometric UI grids, the same stepped typography as the rest of the series. Same family, new focus. The result feels unmistakably Multiverse. But personal. Cinematic enough to hold a room. Precise enough to still read as compliance."
kv:
  src: /assets/inhouse/identity/identity-kv.webp
  width: 206
  height: 206
zipUrl: /assets/inhouse/identity/identity-project-images.zip
cardPreview:
  - src: /assets/inhouse/identity/identity-04-preview.webp
    width: 480
    height: 640
  - src: /assets/inhouse/identity/identity-15-preview.webp
    width: 480
    height: 640
  - src: /assets/inhouse/identity/identity-14-preview.webp
    width: 480
    height: 640
  - src: /assets/inhouse/identity/identity-09-preview.webp
    width: 480
    height: 640
---
